...
class Rider(User):
    ...
    def request_ride(self,ride_sharing, destination, location = None):
        if not self.current_ride:
            rider_request = Ride_Request(self, destination)
            ride_matcher = Ride_Matching(ride_sharing.drivers)
            ride = ride_matcher.find_driver(rider_request)
...
class Ride_Matching:
    ...
    def find_driver(self, rider_request):
        if len(self.available_drivers)  > 0:
            # TODO: find the closest driver of ther rider
...
sakib = Rider("Sakib Khan", "sakib@gmail.com", 975, "mirpu-01", 500 )
niye_jao.add_rider(sakib)
kala_pakhi_driver = Driver("Kala Pakhi", "kalapakhi@gmail.com", 789, "kollanpur")

niye_jao.add_driver(kala_pakhi_driver)

print(niye_jao)

sakib.request_ride( niye_jao,"Malibagh")
